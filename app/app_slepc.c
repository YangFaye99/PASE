...
{
	#if TIME_PASE
	extern double app_axpby_time;
	extern double app_active_time;
	extern double app_mult_time;
	extern int indicator;
	if(indicator){
	app_axpby_time-=DefaultGetWtime();}
	#endif
    ...
    if (x==NULL) {
       ...
       BVScale(y,beta);
		...
		if (x!=y) {
			...
			app_active_time-=DefaultGetWtime();
			}
			#endif
