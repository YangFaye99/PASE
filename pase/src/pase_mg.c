...
int PASE_MULTIGRID_TwoGirdLevel(PASE_MULTIGRID multi_grid)
{
    ...
    int *array_size_g = multi_grid->array_size_g;
    for (idx_level = multi_grid->coarsest_level; idx_level > 0; idx_level--)
    {
        if (array_size_l[idx_level] > 32 && array_size_g[idx_level] > multi_grid->pase_nev)
        {
...
static void get_amg_array_by_gmg(PASE_MULTIGRID *multi_grid, void **A, void **B, void **P, OPS *gcge_ops)
{
#if PRINT_TIME_INFO
    double get_amg_array_time = 0.0;
    double get_A_P_array_time = 0.0;
    ...
    Mat *A_array = NULL, *B_array = NULL, *P_array = NULL;
#if PRINT_INFO
    gcge_ops->Printf("[petsc_gamg] multigrid 层数读入: %d levels\n", (*multi_grid)->num_levels);
#endif
...
int PASE_MULTIGRID_Create_GMG(PASE_MULTIGRID *multi_grid, int num_levels, void **A, void **B, void **P, int pase_nev, OPS *gcge_ops)
{
    ...
    get_amg_array_by_gmg(multi_grid, A, B, P, gcge_ops);
